...
export const VisualGrounding: React.FC<Props> = ({ fileData, boxes }) => {
  ...
  return (
    <div className="w-full bg-anthro-gray/20 border border-gray-800 p-1 group/container">
      ...
      >
        ...
        {fileData.mimeType.startsWith('video') ? (
           ...
           <img 
            src={fileData.previewUrl} 
            alt="Analysis Source" 
            className="w-full h-auto max-h-[70vh] object-contain mx-auto block opacity-80 group-hover/container:opacity-100 transition-opacity duration-700"
           />
        )}
          ...
          {boxes.map((box, idx) => {
            ...
            return (
                ...
                className={`absolute transition-all duration-300 ${
                    hoveredBox === idx ? 'z-20' : 'z-10'
                }`}
                style={getStyle(box)}
                >
                {/* Bracket Style Corners for HUD look */}
                <div className={`absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 transition-colors duration-300 ${hoveredBox === idx ? 'border-white' : colorClass.split(' ')[1]}`}></div>
                <div className={`absolute top-0 right-0 w-3 h-3 border-t-2 border-r-2 transition-colors duration-300 ${hoveredBox === idx ? 'border-white' : colorClass.split(' ')[1]}`}></div>
                <div className={`absolute bottom-0 left-0 w-3 h-3 border-b-2 border-l-2 transition-colors duration-300 ${hoveredBox === idx ? 'border-white' : colorClass.split(' ')[1]}`}></div>
                <div className={`absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 transition-colors duration-300 ${hoveredBox === idx ? 'border-white' : colorClass.split(' ')[1]}`}></div>
                
                {/* Center Fill on Hover */}
                <div className={`absolute inset-0 transition-opacity duration-300 bg-current opacity-0 ${hoveredBox === idx ? 'opacity-10 text-white' : ''}`}></div>

                {/* Label - Fixed to stay inside/near box more cleanly */}
                <div className={`absolute -top-6 left-0 bg-black/90 backdrop-blur-sm px-2 py-1 text-[9px] font-mono whitespace-nowrap border flex items-center gap-2 transition-opacity duration-300 ${
                    hoveredBox === idx ? 'opacity-100 border-white text-white z-50' : `opacity-0 ${colorClass}`
                }`}>
