...
import { Target, User, Box, Building2, Trees, Car } from 'lucide-react';
...
export const VisualGrounding: React.FC<Props> = ({ fileData, boxes, staticMode = false }) => {
  ...
  const handleMediaLoad = (e: React.SyntheticEvent<HTMLImageElement | HTMLVideoElement>) => {
    ...
    if (target.tagName === 'IMG') {
        ...
        w = (target as HTMLVideoElement).videoWidth;
        h = (target as HTMLVideoElement).videoHeight;
    }
  ...
  return (
    <div className={`w-full h-full flex flex-col bg-anthro-gray/20 border-b border-gray-800 p-1 group/container relative ${staticMode ? '' : ''}`}>
      {!staticMode && (
          <div className="flex justify-between items-center px-2 py-1 mb-1 border-b border-gray-800/50 shrink-0 absolute top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-sm">
            <div className="flex items-center gap-2 text-[10px] font-mono text-gray-500">
                <Target className="w-3 h-3 text-signal-orange" />
                <span>VISION_SYSTEM_V4.1</span>
            </div>
      ...
      >
        {/* Constrained Wrapper to match Image Aspect Ratio */}
        <div 
        ...
        >
            ...
            <img 
                src={fileData.previewUrl} 
                alt="Analysis Source" 
                className={`w-full h-full object-contain block ${staticMode ? 'opacity-100' : 'opacity-80 group-hover/container:opacity-100 transition-opacity duration-700'}`}
                onLoad={handleMediaLoad}
            />
            )}

            {/* Grid Overlay - Scoped to Image */}
            ...
            {/* Crosshairs Overlay - Scoped to Image */}
            ...
            {/* Cyber-Scan Animation - Scoped to Image */}
            {!staticMode && (
            ...
            {/* Bounding Boxes Layer - Scoped to Image */}
      ...
      {/* Legend - Only show if not static */}
      {!staticMode && (
        <div className="absolute bottom-4 left-4 right-4 flex gap-2 overflow-x-auto pb-1 scrollbar-none z-30 pointer-events-auto">
            {boxes.map((box, idx) => {
                const colorClass = getCategoryColor(box.category);
                return (
                    <button
                        ...
                        className={`text-left text-[10px] font-mono px-3 py-1 border transition-all flex items-center gap-2 bg-black/80 backdrop-blur-md whitespace-nowrap ${
                            hoveredBox === idx 
